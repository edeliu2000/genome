...
class AdditiveForceArrayVisualizer extends React.Component {
  ...
  handleXLabelSelection = (event) => {
    console.log("event on order change", this);
    this.setState({
       xLabelSelection: event.target.value
    }, this.internalDraw)
  }
  ...
  componentDidMount() {
    ...
    this.svg.on("click", () => {
      //alert("This original index of the sample you clicked is " + this.nearestExpIndex)
      this.props.onClick(this.nearestExpIndex)
    });
  ...
  draw() {
    ...
    console.log("---posDefinedFeatures: ", posDefinedFeatures.length);
    ...
    if (this.props.ordering_keys != null)  {
      options.unshift("order by key");
    }

    if (!options.length === this.state.xLabelOptions.length){
      this.setState({
        xLabelOptions: options
      })
    }
  ...
  internalDraw() {
    ...
    let xsort = this.state.xLabelSelection;


    // Set scaleTime if time ticks provided for original ordering
    let isTimeScale = ((xsort === "order by key") &&
    ...
    if (xsort === "effect similarity") {
      ...
      each(explanations, (e, i) => (e.xmap = i));
    } else if (xsort === "original ordering") {
      explanations = sortBy(this.props.explanations, x => x.origInd);
      each(explanations, (e, i) => (e.xmap = i));
    } else if (xsort === "order by key") {
      ...
      >
        ...
        <FormControl variant="outlined" style={{marginTop:"-3em", marginLeft:"6em", minWidth:130, maxWidth:250}}>
          ...
          <Select
            ...
            id="forceArrayXLabelSelect"
            value={this.state.xLabelSelection}
            onChange={this.handleXLabelSelection}
          >
          {
            this.state.xLabelOptions.map((val, i) => <MenuItem value={val}>{val}</MenuItem>)
          }
          </Select>
        </FormControl>
