...
export default class ModelDetailPicker extends React.Component {
  ...
  constructor(props) {
    ...
    this.loadArtifact = this.loadArtifact.bind(this);
  ...
  handleCloseProps = (event) => {
    this.setState({ openProps: false }, () => {
      // on close restore original meta obj
      if(this.state.original){
        setTimeout(() => {
          this.props.change(this.state.original, this.state.original.artifactType);
        }, 250);
      }
    });
  }
  ...
  loadArtifact = (artifactQuery) => {

    var application = artifactQuery.application || "search";
    ...
    _fetchDataRaw(artifactQuery, (err, artifacts) => {

       if(err) {
         console.log("error on loading artifacts", err);
       ...
       if(artifacts && artifacts.length){
         ...
         this.setState({original: this.state.original !== null ? this.state.original : this.props.meta}, () => {
           this.props.change(artifact, artifact.artifactType);
         });
       }

    }, "/v1.0/genome/modelstore/search?artifactType=" + artifactQuery.artifactType,
  ...
  render(){
    ...
    const steps = isPipeline && this.props.meta.recipeRef && this.props.meta.recipeRef.ref ? JSON.parse(this.props.meta.recipeRef.ref) : null;
    ...
    return (
      ...
      <Dialog
        ...
        <DialogTitle id="dialog-title">{{
          ...
          "evaluation": "Evaluation Spec",
          "evaluationRun": "Evaluation",
          "pipeline": "Pipeline Spec",
          "pipelineRun": "Pipeline Run",
          "deployment": "Deployment"
        }[this.props.meta.artifactType]} Properties</DialogTitle>
        <DialogContent >
        <div>
        ...
        { isPipeline &&
          <PipelineVisualizer
            genomePipeline={steps || this.props.meta.pipelineRef}
        ...
        <List style={{marginTop:"2em"}}>
          ...
          { this.props.meta.dataset &&
          <ListItem divider>
            <ListItemIcon>
              <Icon>arrow_right</Icon>
            </ListItemIcon>
            <ListItemText inset primary="Dataset" secondary={this.props.meta.dataset} />
          </ListItem>
          }
          ...
          { this.props.meta.format &&
          <ListItem divider>
            <ListItemIcon>
              <Icon>arrow_right</Icon>
            </ListItemIcon>
            <ListItemText inset primary="Format" secondary={this.props.meta.format} />
          </ListItem>
          }
        ...
        <ModelValidation
          ...
          validationTarget={{
            ...
            refType: this.props.meta.artifactType || "modelArtifact"}}
          application={ this.props.meta.application || ""}
          validation={{
            canonicalName: this.props.meta.canonicalName || "",
            status: this.props.meta.status || null,
            dimension: this.props.meta.dimension || null,
            tasks: this.props.meta.tasks || null,
            metrics: this.props.meta.metrics || null,
            validationTarget: {
              ref:this.props.meta.targetRef,
              refType: this.props.meta.targetRefType || "modelArtifact"
            }
          }}
        ...
        { this.props.meta.artifactType in {"model":1, "modelArtifact":1} &&
        <ModelVisualizer
          canonicalName={ this.props.meta.canonicalName || ""}
          application={ this.props.meta.application || ""}
